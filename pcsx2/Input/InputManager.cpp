...
// User-binding reverse lookups for controller navigation. Populated at ReloadBindings time from
// the user's pad configuration. User bindings take priority over any static table defaults passed
// through InvokeEvents, so remapped controllers are always respected.
		...
		switch (bi.bind_type)
		{
			...
			case InputBindingInfo::Type::HalfAxis:
			{
				...
				if (!bindings.empty())
				{
					...
					// Build reverse maps for controller navigation. User bindings take priority over
					// any static table defaults, so remapped controllers are always respected.
